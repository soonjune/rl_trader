...
class open_api(QAxWidget):
    ...
    def variable_setting(self):
        ...
        if self.account_number == cf.real_account:  # 실전
            self.num = cf.imi1_simul_num
            self.num = cf.real_simul_num
    ...
    def rate_check(self):
        ...
        for k in range(num):
            ...
            holding_amount, purchase_price, present_price, valuation_profit, float(rate), item_total_purchase, code, 0))
...
class open_api(QAxWidget):
    ...
    def account_info(self):
        logger.debug("account_info 함수에 들어왔습니다!")
        account_number = self.get_login_info("ACCNO")
        for acc in account_number.split(';'):
            if acc == cf.acct_no:
